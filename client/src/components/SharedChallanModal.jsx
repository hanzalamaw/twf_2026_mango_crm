...
import { formatMangoOrderLabel } from '../utils/mangoOrderFormat';
...
const MANGO_COLUMNS = ['Description', 'Order ID', 'Customer Name', 'Contact', 'Alt Contact', 'Type', 'Qty', 'Weight (kg)', 'Status'];
...
export default function SharedChallanModal({
  ...
  const columns = variant === 'mango' ? MANGO_COLUMNS : ['Description', 'Order', 'Contact', 'Alt Contact', 'Shareholder', 'Type', 'Cow #', 'Hissa #', 'Slot', 'Status'];
      ...
      >
        ...
        <div style={{ marginBottom:'18px' }}>
          <div style={{ border:'1px solid #E4E4E4', borderRadius:'10px', overflow:'hidden', background:'#F5F5F5' }}>
            <div style={{ display:'grid', gridTemplateColumns:'130px 1fr', gap:'0', padding:'14px 16px', fontSize:'12px', lineHeight:1.5 }}>
              <span style={{ fontWeight:'700', color:'#555' }}>Order Summary:</span>
        ...
        <div className="modal-table-scroll ops-modal-table-wrap" style={{ border:'1px solid #F0F0F0', borderRadius:'10px' }}>
          <table className="modal-orders-table" style={{ fontSize:'12px', borderCollapse:'collapse' }}>
            ...
            <tbody>
              {orders.length === 0 ? (
                <tr><td colSpan={columns.length} style={{ padding:'22px', textAlign:'center', color:'#aaa' }}>No orders linked.</td></tr>
              ) : orders.map((o, i) => (
                variant === 'mango' ? (
                  <tr key={o.order_id || i} style={{ borderBottom:'1px solid #F0F0F0', background: i % 2 === 0 ? '#fff' : '#FAFAFA' }}>
                    ...
                    <td style={{ padding:'9px 10px', color:'#333', fontWeight:'500', ...wrapCell }}>{o.name || o.booking_name || '—'}</td>
                    ...
                    <td style={{ padding:'9px 10px', color:'#555', ...wrapCell }}>{formatMangoOrderLabel(o)}</td>
                    <td style={{ padding:'9px 10px', color:'#555', ...nowrapCell }}>{o.quantity ?? '—'}</td>
                    <td style={{ padding:'9px 10px', color:'#555', ...nowrapCell }}>{o.weight != null && o.weight !== '' ? o.weight : '—'}</td>
                  ...
                  <tr key={o.order_id || i} style={{ borderBottom:'1px solid #F0F0F0', background: i % 2 === 0 ? '#fff' : '#FAFAFA' }}>
                    ...
                    <td style={{ padding:'9px 10px', color:'#555', ...nowrapCell }}>{o.contact || '—'}</td>
                    <td style={{ padding:'9px 10px', color:'#555', ...nowrapCell }}>{o.alt_contact || '—'}</td>
                    <td style={{ padding:'9px 10px', color:'#333', fontWeight:'500', ...wrapCell }}>{o.shareholder_name || o.booking_name || o.name || '—'}</td>
