  ...
  return (
    <>
      ...
      <div className="om-root" style={{ padding: '19px', fontFamily: "'Poppins','Inter',sans-serif", display: 'flex', flexDirection: 'column', minHeight: 0, height: '100%', overflow: 'hidden', boxSizing: 'border-box' }}>
        ...
        {!loading && <div style={{ fontSize:'10px', color:'#999', marginBottom:'8px', flexShrink:0 }}>Showing {groups.length} of {totalGroups} special request groups matching filters</div>}
        ...
        <div className="om-table-wrap" style={{ flex:1, minHeight:0, overflow:'auto', borderRadius:'10px', border:'1px solid #ececec' }}>
          {loading ? (
            ...
            <div style={{ padding:'40px', textAlign:'center', color:'#666', fontSize:'11px' }}>{totalGroups===0 && !searchDebounced && !challanDebounced && !filterSlots.length && !filterStatus.length && !filterRider && !filterOrderType.length && !scanMatchToken ? 'No special request challans for this batch.' : 'No special request rows match the current filters.'}</div>
          ) : (
