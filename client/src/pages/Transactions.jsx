...
export default function Transactions() {
  ...
  const fetchFilters = useCallback(async () => {
    try {
      const res = await fetch(`${API}/booking/orders/filters?year=all`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const fetchSummary = useCallback(async () => {
    try {
      const res = await fetch(`${API}/booking/transactions`, { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
  ...
  const fetchOrdersSummary = useCallback(async () => {
    try {
      ...
      if (res.ok) setOrdersSummary(await res.json());
    } catch (e) {
      console.error(e);
    }
  }, [token, yearFilter]);
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/booking/orders?${params.toString()}`, { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
        const data = await res.json();
  ...
  }, [token, yearFilter, appliedTypes, page, debouncedSearch, paymentStatusFilter]);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const toExport = ids.length > 0 ? all.filter((r) => ids.includes(r.order_id)) : all;
      if (!toExport.length) {
  ...
  return (
    ...
    <div className="txn-root" style={{ padding: '19px', fontFamily: "'Inter', 'Segoe UI', system-ui, -apple-system, sans-serif", display: 'flex', flexDirection: 'column', minHeight: 0, height: '100%', overflow: 'hidden', boxSizing: 'border-box' }}>
      ...
      <div className="txn-cards" style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '16px', flexShrink: 0 }}>
        {/* Bank Total card (TWF + TW Traders) */}
        <div className="txn-card" style={{ flex: '1 1 160px', minWidth: '160px', padding: '14px 12px', borderRadius: '10px', border: '1px solid #f1f1f1', background: '#fff', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', display: 'flex', alignItems: 'center', gap: '8px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '2px' }}>
            <div className="txn-card-label" style={{ fontSize: '11px', fontWeight: '400', color: '#6b7280' }}>Bank Total</div>
            <div className="txn-card-amount" style={{ fontSize: '18px', fontWeight: '600', color: '#111827', lineHeight: '1.2' }}>
              {amountVisible
                ? <span>{formatAmount(bankTotalAmount)}</span>
                : <span style={{ filter: 'blur(6px)', userSelect: 'none', display: 'inline-block', minWidth: '120px', background: 'rgba(0,0,0,0.03)', borderRadius: '10px', padding: '6px 10px' }}>{formatAmount(bankTotalAmount)}</span>}
            </div>
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            <div className="txn-modal-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 16px', marginBottom: '13px', fontSize: '10px', padding: '8px 10px', background: '#f5f5f5', borderRadius: '6px', border: '1px solid #e8e8e8' }}>
              ...
              <div><span style={{ color: '#666' }}>Name</span><div style={{ fontWeight: '600' }}>{modalOrder.name ?? '—'}</div></div>
