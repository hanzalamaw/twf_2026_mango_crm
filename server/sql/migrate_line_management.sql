-- Line Management (independent tables + Operations permission)
-- Run once against your CRM database. Not linked to slaughter or other modules.
...
CREATE TABLE IF NOT EXISTS line_records (
  ...
  CONSTRAINT fk_line_group FOREIGN KEY (group_id)
    REFERENCES line_groups (group_id) ON DELETE CASCADE,
...
ALTER TABLE roles
  ADD COLUMN operation_line_management TINYINT(1) NOT NULL DEFAULT 0
  AFTER operation_slaughter_management;

-- If you already created line_records with cow/goat enum, run:
