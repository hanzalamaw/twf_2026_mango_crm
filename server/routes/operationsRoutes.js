...
import {
  dayLabelToNumber,
  fetchDashboardSlaughterPacking,
} from "../utils/dashboardSlaughterPacking.js";
...
export const registerOperationsRoutes = (app, db, verifyToken, io = null) => {
  ...
  app.get("/api/operations/dashboard/stats", verifyToken, async (req, res) => {
    try {
      ...
      const { slaughter, packing } = await fetchDashboardSlaughterPacking(db, day);
  ...
  app.get("/api/operations/dashboard/slaughter-packing", verifyToken, async (req, res) => {
    try {
      const flags = await assertSub(req, res, (f) => f.operation_general_dashboard || f.operation_management);
      if (!flags) return;
      const { day } = req.query;
      const { slaughter, packing } = await fetchDashboardSlaughterPacking(db, day);
      res.json({ slaughter, packing });
    } catch (error) {
      logError("OPERATIONS", "Dashboard slaughter-packing error", error);
      res.status(500).json({ message: "Server error" });
    }
  });
